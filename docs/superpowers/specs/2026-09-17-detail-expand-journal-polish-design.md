...
6. **Write** mode stays on the same `QWebEngineView`: `.txt` is `contenteditable`. Do not switch to `QPlainTextEdit` for typing — Qt’s rasterizer cannot match Chromium. `JournalEditor` remains the in-memory model for tests and save fallback. Ctrl+S / Esc take Chromium text when a row actually has text; do not discard typed rows just because a dirty flag stayed false. If JavaScript returns no rows, or every row is blank while the editor still has text, keep the editor model — a blank `innerText` read must not wipe the journal. Empty new stamps still drop only after that commit.
