...
6. **Write** mode switches to `JournalEditor` (`QPlainTextEdit`). Chromium stays **read-only**. Ctrl+I used to type in `contenteditable`; Ctrl+S could not reliably read that page, so typed text vanished. Typing and saving must use the same editor again. Empty new stamps still drop only after that commit.
