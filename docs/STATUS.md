...
**UI 成熟度打磨**（规格已批准，待按 TDD 实现）

- 保持功能、主题、公开接口、快捷键、数据格式和窗口几何不变
- 增加日志工作流提示与保存状态、卡片键盘/无障碍操作、空状态和托盘隐藏语义
- 删除改为原标题栏内两步确认，不增加弹窗或新按钮
...
写 TDD 实施计划；按日志状态、键盘/无障碍、删除确认、空状态四个任务实现并回归；重建冻结包、刷新桌面快捷方式。
