...
- 验证：`python -m pytest -v` → **25 passed**（包装测试含在内）。`dist\Tasker\Tasker.exe` 由 `build_windows.ps1` 生成，不进 git。

## 下一步

1. 源码试用：`.\scripts\setup.ps1 -SkipBuild`
2. 冻结 exe：`.\scripts\build_windows.ps1`，再双击 `dist\Tasker\Tasker.exe`
3. 交互要改再开规格
