...
def dock_style() -> str:
    return (
        ...
        f"QLineEdit#search{{background:{CARD};color:{INK};"
        f"border:1px solid {LINE};border-radius:12px;padding:0 12px;font-size:15px;"
        f"selection-background-color:{COBALT};}}"
        f"QLineEdit#search:hover{{border:1px solid {MUTED};}}"
        f"QLineEdit#search:focus{{border:2px solid {COBALT};padding:0 11px;}}"
